    ...
    <main class="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <section class="bg-white border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                ...
                <div class="text-xs text-slate-500">Enter the password from `.env` (`ADMIN_PASSWORD`). If not set, it will fall back to `COMPRESS_KEY`.</div>
            </div>
                    ...
                    <article class="bg-white border border-slate-200 rounded-xl overflow-hidden" data-card-pin-id="<?= (int) $id ?>">
                        ...
                        <div class="p-2 space-y-2">
                            ...
                            <div class="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-2 py-1">
                                ...
                                <label class="inline-flex items-center cursor-pointer select-none" title="Enable/disable automatic redirect to the affiliate link on the preview page">
                                    <input
                                        type="checkbox"
    ...
    (function(){
        ...
        async function setRedirectEnabled(pinId, enabled, prevEnabled, inputEl, statusEl) {
            ...
            try {
                ...
                if (statusEl) statusEl.textContent = finalEnabled ? 'Redirect enabled' : 'Redirect disabled';
            } catch (err) {
                if (inputEl) {
